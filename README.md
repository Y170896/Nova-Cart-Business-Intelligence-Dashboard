...
- Build an interactive Power BI dashboard for business decision-making
  ...
  3. SQL Analysis
  39+ business SQL queries were written in [`NovaCart_SQL_Analysis_Queries.sql`](SQL%20Queries/NovaCart_SQL_Analysis_Queries.sql) to answer real business questions, covering:     Customer Analysis · Revenue Analysis · Product Analysis · Brand Analysis · Category Analysis · Order Analysis · Return Analysis · Payment Method Analysis · Monthly Sales Trend ·   Customer Segmentation · Ranking Analysis · Window Functions · Revenue Contribution · Top Customers · Top Products

  Sample queries — Top 3 Products in Each Category (DENSE_RANK):
  WITH X AS(
  ...
  Cumulative Revenue Trend (Running Total):
  SELECT Order_Date,
  SUM(Order_Value) OVER(ORDER BY Order_Date) Cumulative_Revenue
  FROM tbl_orders;

  Customers Spending Above Average (Correlated Subquery):
...
- Wallet and UPI emerged as the most preferred payment methods, each contributing ₹1.4M in revenue
 ...
 Full detailed findings are documented in the [`NovaCart Report.pdf`](Report/NovaCKey Business Insights
